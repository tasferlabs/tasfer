...
/** Directory this host owns, distinct from every other Tasfer install's. */
const DATA_DIR_NAME = "tasfer-cli";
 ...
 *
 * Named for this host and not the product: a data directory *is* an identity,
 * and two replicas sharing one are two devices with the same public key. They
 * evict each other's signaling socket on every topic — see the relay's
 * "replaced by a newer connection" — and never see each other, because a peer
 * with your own key is not a peer. Every install gets its own directory so
 * that cannot happen by default.
 */
