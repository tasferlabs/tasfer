...
const buttonVariants = cva(
  "button-square-ripple relative isolate overflow-hidden cursor-pointer disabled:cursor-not-allowed group/button inline-flex shrink-0 items-center justify-center rounded-md border border-transparent bg-clip-padding text-sm font-medium whitespace-nowrap transition-all outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
...
function Button({
  ...
  const handlePointerDown: React.PointerEventHandler<HTMLButtonElement> = (
    ...
    if (!window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
      event.currentTarget.removeAttribute("data-ripple-active");
      void event.currentTarget.offsetWidth;
      event.currentTarget.setAttribute("data-ripple-active", "");
    }

    onPointerDown?.(event);
  };

  const handleAnimationEnd: React.AnimationEventHandler<HTMLButtonElement> = (
    event,
  ) => {
    if (event.animationName === "button-square-ripple") {
      event.currentTarget.removeAttribute("data-ripple-active");
    }
