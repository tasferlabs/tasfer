...
function makeReplicator(trustedPeers: Peer[] = []) {
  ...
  const host = { getTrustedPeers } as unknown as ReplicatorHost;

  const replicator = new Replicator(network, host);
  return { replicator, flush, pause, resume, getTrustedPeers };
}
...
describe("Replicator pause/resume", () => {
  ...
  it("resume() connects a trusted peer that has no topic yet", async () => {
    ...
    const { replicator, getTrustedPeers } = makeReplicator([peer]);
