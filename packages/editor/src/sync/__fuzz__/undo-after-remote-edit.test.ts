...
describe("undo after remote edit", () => {
  it("captured inverse removes the bold without disturbing remote text", () => {
    // Two peers. The per-editor CRDT binding holds the HLC + id-gen that the
    // per-op helpers (crdt-helpers, inverse) consume via
    // binding.nextId()/binding.getClock(). To make this test exercise the
    // same HLC the inverses use, we build peer A's ops through this binding.
    const binding = createCRDTbinding("undo-trace", "p001");
    const pageId = binding.pageId;

    const peerA = createSyncEngine(binding);
    ...
    expect(visibleText(peerA.getState())).toBe("hello");
    expect(visibleText(peerB.getState())).toBe("hello");

    // 2. Peer A bolds chars 0..4 (the "hell" prefix). Build the op with the
    //    binding's HLC (the same one inverse capture will consume) so the
    //    LWW ordering between the captured inverse and the original op is
    //    the natural one (inverse's clock > op's clock).
    const pageBeforeBold = peerA.getState();
    const blockOnA = pageBeforeBold.blocks.find((b) => b.id === blockId);
    expect(blockOnA && isTextualBlock(blockOnA)).toBe(true);
    if (!blockOnA || !isTextualBlock(blockOnA)) return;
    ...
    // CAPTURE the inverse against pageBeforeBold — same as recordUndoOps
    // does in production.
    ...
    // 3. Peer B appends " world" at the end of the paragraph — a remote
    //    edit that doesn't touch the bolded chars; it just adds 6 chars.
    const blockOnB = peerB.getState().blocks.find((b) => b.id === blockId);
    expect(blockOnB && isTextualBlock(blockOnB)).toBe(true);
    if (!blockOnB || !isTextualBlock(blockOnB)) return;
    ...
    expect(visibleText(peerA.getState())).toBe("hello world");
    ...
    for (const span of finalBlock.formats) {
      ...
      for (let i = startIdx; i <= endIdx; i++) {
        if (originalBoldCharIds.has(sequenceIds[i])) {
          stillBolded.push(sequenceIds[i]);
        }
      }
    }
    expect(stillBolded).toEqual([]);

    // Undo of bold must not disturb the text.
    expect(visibleText(finalA)).toBe("hello world");

    // And the inverse should be idempotent — applying it again is a no-op
    // (the bold span is already gone).
    const reapplied = applyOps(finalA, refreshOps(capturedInverses, binding));
    expect(reapplied.blocks.find((b) => b.id === blockId)).toBeDefined();
  });
});
