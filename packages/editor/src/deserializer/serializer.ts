...
export function serializeToMarkdown(
  ...
  const serializedBlocks = blocks.map((block, index) => {
    ...
    if (isListBlock(block)) {
      ...
      const segments = groupCharsForSerialization(
        block.charRuns,
        block.formats,
      );
  ...
  if (
    ...
    const hasContent =
      isListBlock(lastBlock) ||
      lastBlock.type === "heading1" ||
      lastBlock.type === "heading2" ||
      lastBlock.type === "heading3" ||
