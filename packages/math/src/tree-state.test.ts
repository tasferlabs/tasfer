...
import { INSERT_MATH_COMMAND, RESIZE_MATH_MATRIX } from "./MathNode";
import {
...
import { TEXT_INPUT } from "@tasfer/editor/action-bus";
...
import { applyStructuredEdits } from "@tasfer/editor/sync/structured-content";
...
describe("tree-backed display math state integration", () => {
  ...
  it("keeps typing after an accent commit under the accent", () => {
    ...
    const state = committed.actionBus.dispatchState(
      INSERT_MATH_COMMAND,
      committed,
      { text: String.raw`\vec{}`, caretOffset: 5 },
    ).state;

    expect(treeSource(typeText(state, "xy").state)).toBe(String.raw`\vec{xy}`);
  });
