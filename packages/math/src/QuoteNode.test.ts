...
import { type QuoteBlock } from "@tasfer/editor/nodes/QuoteNode";
...
describe("cardJoinFlags (adjacent card coupling)", () => {
  ...
  // The code block joins this same group; its node lives in `@tasfer/code`,
  // so that pairing is covered by that package's suite.
  it("tiles across card types — a quote meets an adjacent math block", () => {
    const blocks = [quote("q0"), math("m1"), quote("q2")];
    expect(join(blocks, 0)).toEqual({ joinTop: false, joinBottom: true });
    expect(join(blocks, 1)).toEqual({ joinTop: true, joinBottom: true });
    expect(join(blocks, 2)).toEqual({ joinTop: true, joinBottom: false });
  });
