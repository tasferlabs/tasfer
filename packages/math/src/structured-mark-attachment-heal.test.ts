...
import { convertToList, mergeBlocksOps } from "@tasfer/editor/actions/actions";
...
import { applyOps, rebuildState } from "@tasfer/editor/sync/reducer";
...
describe("structured mark attachment healing", () => {
  it("re-adopts a dropped attachment when merging instead of stranding it", () => {
    ...
    const sourceIndex = 2;
    const contentId = markContentIds(
    ...
    const joined = merged.newPage.blocks.find(
      (block) =>
        block.id === state.document.page.blocks[0].id && !block.deleted,
    );
