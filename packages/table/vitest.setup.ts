...
if (typeof g.document === "undefined") {
  ...
  g.document = {
    ...
    createElement: () => ({
      // Width proportional to length (rather than a constant), so the table's
      // column-fitting and wrap geometry can be asserted on: a longer cell must
      // measure wider than a short one for any of it to mean anything. Font
      // metrics fall back to their ratio defaults, which is what the layout
      // already does when a real canvas reports none.
      getContext: () => ({
    ...
    addEventListener: () => {},
    // The scrollbar's iOS safe-area probe measures a node it appends to the
    // body; the pointer paths that hit-test chrome go through it.
